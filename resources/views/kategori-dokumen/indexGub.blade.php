        ...
        <div class="row g-4">
            ...
            <div class="col-lg-9 col-12 order-lg-1 order-2">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="mb-0">Dokumen Peraturan Gubernur</h4>
                    ...
                    <div class="card-body p-2 p-md-3">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td colspan="4" class="text-center text-muted py-4">
                                            Belum ada dokumen Peraturan Gubernur yang tersedia.
                                        </td>
                                    </tr>
                                    @endforelse
                                </tbody>
                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap mt-3 gap-2">
                            <div class="d-flex gap-1">
                                @if ($peraturanGubernur->onFirstPage())
                                    <span class="btn btn-outline-secondary disabled">&laquo;</span>
                                @else
                                    <a href="{{ $peraturanGubernur->previousPageUrl() }}" class="btn btn-outline-secondary">&laquo;</a>
                                @endif

                                <form method="GET" class="d-flex" onsubmit="return goToPage(this)">
                                    <input type="number" name="page" min="1" max="{{ $peraturanGubernur->lastPage() }}" value="{{ $peraturanGubernur->currentPage() }}" class="form-control form-control-sm text-center" style="width:60px;">
                                </form>

                                @if ($peraturanGubernur->hasMorePages())
                                    <a href="{{ $peraturanGubernur->nextPageUrl() }}" class="btn btn-outline-secondary">&raquo;</a>
                                @else
                                    <span class="btn btn-outline-secondary disabled">&raquo;</span>
                                @endif
                            </div>
                            <div>
                                <small>Page {{ $peraturanGubernur->currentPage() }} of {{ $peraturanGubernur->lastPage() }}</small>
                            </div>
                        </div>
