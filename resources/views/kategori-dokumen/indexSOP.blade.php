                    ...
                    <div class="card-body p-2 p-md-3">
                        <div class="table-responsive">
                            <table class="table table-hover table-striped align-middle mb-0">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                    <tr>
                                        <td colspan="4" class="text-center text-muted py-4">Belum ada dokumen SOP yang tersedia.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap mt-3 gap-2">
                            <div class="d-flex gap-1">
                                ...
                                @else
                                    <a href="{{ $SOP->previousPageUrl() }}" class="btn btn-outline-secondary">&laquo;</a>
                                @endif

                                <form method="GET" class="d-flex" onsubmit="return goToPage(this)">
                                    <input type="number" name="page" min="1" max="{{ $SOP->lastPage() }}" value="{{ $SOP->currentPage() }}" class="form-control form-control-sm text-center" style="width:60px;">
                                </form>

                                @if ($SOP->hasMorePages())
                                    <a href="{{ $SOP->nextPageUrl() }}" class="btn btn-outline-secondary">&raquo;</a>
                                @else
                                    <span class="btn btn-outline-secondary disabled">&raquo;</span>
                                @endif
                            </div>
                            <div><small>Page {{ $SOP->currentPage() }} of {{ $SOP->lastPage() }}</small></div>
                        </div>
